...
export default async function MovieReviewsPage({
  ...
  const [movie, reviewsResponse] = await Promise.all([
    ...
    getMovieReviews(parsedMovieId, page),
  ])
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#030711]">
      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl px-4 pb-8 pt-24 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-300/80">Movie reviews</p>
        ...
        {reviewsResponse.reviews.length ? (
          ...
          <p className="text-sm text-slate-300/80">No reviews available for this movie.</p>
        )}

        <div className="mt-6 flex items-center justify-between">
          {canGoPrev ? (
            <Link
              href={`/movies/${movie.id}/reviews?page=${reviewsResponse.page - 1}`}
              className="rounded-full bg-white/10 px-3 py-1.5 text-xs font-medium text-white hover:bg-white/20"
            >
              Previous page
            </Link>
