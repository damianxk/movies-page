import { type SeriesReview } from "@/features/series/types/series-reviews"
import { MovieSectionTitle } from "@/features/movies/components/movie-section-title"

type SeriesDetailsReviewsProps = {
  seriesId: number
  reviews: SeriesReview[]
  totalResults: number
}

function getReviewAuthor(review: SeriesReview) {
...
export function SeriesDetailsReviews({
  seriesId,
  reviews,
  totalResults,
}: SeriesDetailsReviewsProps) {
  const visibleReviews = reviews.slice(0, 3)

  return (
    <section id="reviews" className="scroll-mt-32 py-2">
      <MovieSectionTitle title="Reviews" count={totalResults} href={`/series/${seriesId}/reviews`} />
      ...
      {visibleReviews.length ? (
        ...
        <p className="mt-4 text-sm text-slate-300/80">No reviews available for this series.</p>
      )}
    </section>
  )
}
